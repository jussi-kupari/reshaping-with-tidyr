...
# Note that using 'names_sep' requires a 'names_to' length > 1
```
...
# Using complete instead!
percentages %>% 
  complete(year, type, fill = list(percentage = 0)) %>% 
  pivot_wider(names_from = c(year, type), values_from = percentage)
```
...
# Using complete instead!
daily %>% 
  complete(day, type, fill = list(value = 0)) %>% 
  pivot_wider(
    names_from = type, 
    values_from = value)
```
