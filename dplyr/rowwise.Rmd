...
*   Without argument names: you could call functions that input and output 
    ...
    This has been superseded by `pick()` plus `reframe()`, a variant of
    `summarise()` that can create multiple rows and columns per group.
...
*   With arguments: it worked like `mutate()` but automatically wrapped every
    ...
    it can be replaced by `summarise()` and `pick()`. 
